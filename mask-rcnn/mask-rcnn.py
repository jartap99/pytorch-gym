...
class PennFudanDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # masks are not yet converted to RGB yet as each color corresponds to different instance
        # 0 being background
        
        mask = Image.open(mask_path)
        # convert PIL image to number array
        mask = np.array(mask)
...
def method1_finetune_from_pretrained():
...
def method2_modify_to_Add_diff_backbone():
